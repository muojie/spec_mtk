...
uchar	SendBuf_1[20]={0xA3,0x12,0x01,0x01,0x02,0x03,0x04,0x05,
						...
						0x55,0x66,0x77,0x88};		
/*发送命令实现DES加密演示命令应返回 A3  08 6C5E94DCADD39F1D */
...
void SendChar_1(uchar m,uchar Send[])
{
    uchar i,n,flag,sec,SendChar;
		...
		{	
			...
			SendChar = 	Send[n];	 		//将发送的字节放在SendChar	 
		   	IO  = 0;
			for(i=0;i<8;i++)			   //发送一个字节  该循环一次的时间大约17us 左右
			{
				for(sec=0;sec<7;sec++);
				if(SendChar & 0x01)
					IO = 1;
...
void main()
{
	...
	SendChar_1(20,SendBuf_1)	;	//将SendBuf_1的长度和首地址传给SendChar_1(uchar m,uchar Send[])
	ReceArt_1(10);					//返回数据应为A3  08 6C5E94DCADD39F1D 
	while(1);
}
