...
class PowerManagerService extends IPowerManager.Stub
    ...
    private static final int LIGHT_SENSOR_DELAY = 1200;
    ...
    private boolean mWaitKeyguardDraw = false;
    ...
    private int mLightSensorScreenBrightnessWait = -1;
    ...
    private static final boolean IGNORE_PREVENT_SCREEN_ON_FOR_SET_SCREEN_STATE = true;
    private SyncKeyguard mSyncKeyguard = new SyncKeyguard();// add this for delay animating on before keyguard is draw done.	
    private boolean mDoDelayLightAnimation = false;

    /*  1. Backlight Behavior Change Feature Since Android 2.3.
        2. FEATURE_CHANGE_BKL_ENABLED is for feature that align BKL behavior
           FEATURE_DISABLE_LS_ENABLED is for feature that disable LS at proper time.
        3. Both features are only valid for platforms that supports Light Sensor.
        4. To make development and customization convenience, these two features are independent. 
           So you can enable both or any one of them according to your need.
    */
    private static final boolean FEATURE_ALIGN_BKL_ENABLED = true;
    private static final boolean FEATURE_DISABLE_LS_ENABLED = true;
    private boolean mIsLowBattery = false;
    private static boolean mShutdownConfig = false;
    ...
    */
    ...
    private final class BatteryReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            synchronized (mLocks) {
                ...
                //Begin for feature that disable Light Sensor at proper time 
                boolean wasLowBattery = mIsLowBattery;
                mIsLowBattery = batteryIsLow();
                if (mUseSoftwareAutoBrightness && FEATURE_DISABLE_LS_ENABLED && mIsLowBattery != wasLowBattery && mDoneBooting) {
                    if (mDebugLightSensor) {
                        Slog.d(TAG, "<<<<< FEATURE_DISABLE_LS >>>>> Battery Power Changed: mIsLowBattery=" + mIsLowBattery);
                    }
                    if ((mPowerState & SCREEN_ON_BIT) != 0 && mAutoBrightessEnabled) {
                        enableLightSensor(!mIsLowBattery);	
                        if (mIsLowBattery) {
                            if (!FEATURE_ALIGN_BKL_ENABLED) {
                                mButtonLight.turnOff();
                                mKeyboardLight.turnOff();
                            }
                            mLightSensorValue = -1;
                            mHighestLightSensorValue = -1;
                            mLightSensorScreenBrightnessWait = -1;
                        }
                    }
                }
                //End for feature that disable Light Sensor at proper 
    ...
    private final class SDHotPlugReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            synchronized (mLocks) {
                ...
                if (intent.getAction().equals(Intent.ACTION_SD_INSERTED) || (mPowerState & SCREEN_ON_BIT) != 0) {
                    forceUserActivityLocked();
                }
            }
        }
    ...
    private final class KeyguardDoneDrawReceiver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            synchronized (mLocks) {
                if (mSpew)	Slog.d(TAG, "KeyguardDoneDrawReceiver onReceive DONE_DRAW");
								
                if (mWaitKeyguardDraw == true)
                {
                    screenOffFinishedAnimatingLocked(mScreenOffReason);
                    mWaitKeyguardDraw = false;									
                }
                synchronized(mSyncKeyguard){
                    if(!mSyncKeyguard.mKeyguardDoneDraw){
                        mSyncKeyguard.mKeyguardDoneDraw = true;
                        mSyncKeyguard.notify();
                        if(mSpew) Slog.d(TAG, "KeyguardDoneDrawReceiver receives DONE_DRAW, do notify");								
                    }
                }
            }
        }
    ...
    private final class SyncKeyguard{
        public boolean mKeyguardDoneDraw;
        SyncKeyguard(){
            mKeyguardDoneDraw = true;
        }
    }
    ...
    void initInThread() {
       ...
       ContentResolver resolver = mContext.getContentResolver();
        ...
        // register for keyguard done drawing notification
        filter = new IntentFilter();
        filter.addAction(Intent.ACTION_SEND);
        mContext.registerReceiver(new KeyguardDoneDrawReceiver(), filter, "com.android.internal.policy.impl.KeyguardViewMediator.DONE_DRAW", null);
        
        //register for SD Hot Plug notification Start
        SDHotPlugReceiver sdReceiver = new SDHotPlugReceiver();
        filter = new IntentFilter();
        filter.addAction(Intent.ACTION_SD_INSERTED);
        mContext.registerReceiver(sdReceiver, filter);
        ...
        mContext.registerReceiver(sdReceiver, filter);
        //register for SD Hot Plug notification Stop
    ...
    private void releaseWakeLockLocked(IBinder lock, int flags, boolean death) {
        ...
        if (isScreenLock(wl.flags)) {
            if ((wl.flags & LOCK_MASK) == PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK) {
                ...
                if ((wl.flags & PowerManager.ON_AFTER_RELEASE) != 0) {
                    if ( !isScreenOn() && !mSyncKeyguard.mKeyguardDoneDraw ) {
                        mSyncKeyguard.mKeyguardDoneDraw = true;
                    }
        ...
        //Add for WakeLockDebugTool	
        if (mIsWakeLockBGDbgModeRunning) {
            getDynamicWakeLockLocked(wl, false); 
        }		
    }
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        synchronized (mLocks) {
            ...
            pw.println("  mLightSensorScreenBrightness=" + mLightSensorScreenBrightness
                    + " mLightSensorScreenBrightnessWait=" + mLightSensorScreenBrightnessWait
        ...
        synchronized (mLocks) {
            if (prevent) {
                ...
                if (!mProximitySensorActive && (mPowerState & SCREEN_ON_BIT) != 0) {
                    ...
                    if (!IGNORE_PREVENT_SCREEN_ON_FOR_SET_SCREEN_STATE) {
                        int err = setScreenStateLocked(true);   // will turn on LCM driver
                        if (err != 0) {
                            Slog.w(TAG, "preventScreenOn: error from setScreenStateLocked(): " + err);
                        }
                    }
                    //
                    // Reschedule light animation here to guarantee the in-call UI
                    // is ready to show before backlight is turned on.
                    //
                    mScreenOffHandler.removeCallbacks(mScreenBrightness.mLightAnimator);
                    mScreenOffHandler.post(mScreenBrightness.mLightAnimator);
                }
    ...
    //new method: do clear the light sensor value after backlight off, and before screen off
    //old method: do clear the light sensor value when screen off.
    //new method is neccessary, because screen off will be delayed until keyguard draw done, and the light sensor value may not be cleared before backlight on again, this is captured when quickly press power key twice time, 
    private void clearLightSensorValue()
    {
        // clear current value so we will update based on the new conditions
        // when the sensor is reenabled.
        mLightSensorValue = -1;
        if (mProximitySensorEnabled == false) {      //when screen off during phone call, prevent screen dim
        	// reset our highest light sensor value when the screen turns off
        	Slog.d(TAG, "clearLightSensorValue: mHighestLightSensorValue = -1");
        	mHighestLightSensorValue = -1;
        }
        mLightSensorScreenBrightnessWait = -1;
    }
    ...
    private int setScreenStateLocked(boolean on) {
        int err = 0;
        if (FeatureOption.MTK_IPO_SUPPORT == true) {
            if ("1".equals(SystemProperties.get("sys.ipo.shutdown"))) {
                if(false == mShutdownConfig) {
                    SystemProperties.set("sys.ipo.pmsShutdown","1");
                    mShutdownConfig = true;
                }
                err = 0;
            } else {
                mShutdownConfig = false;
                err = Power.setScreenState(on);
            }
        } else {
            err = Power.setScreenState(on);
        }
        Slog.w(TAG, "setScreenState:" + on);
        if (err == 0) {
            mLastScreenOnTime = (on ? SystemClock.elapsedRealtime() : 0);
            if (mUseSoftwareAutoBrightness) {
                if (!FEATURE_DISABLE_LS_ENABLED || (mAutoBrightessEnabled && !mIsLowBattery) || (mLightSensorEnabled && !on)) {
        ...
        synchronized (mLocks) {
            ...
            if (oldScreenOn != newScreenOn) {
                if (newScreenOn) {
                    ...
                    if (mPreventScreenOn && !IGNORE_PREVENT_SCREEN_ON_FOR_SET_SCREEN_STATE) {
                        if (mSpew) {
                    ...
                    if (!mScreenBrightness.animating) {
                        if(!KeyguardViewMediator.isKeyguardEnabled())//turn screen off in default flow, only when keyguard is disabled 
                        {
                             clearLightSensorValue();
                             err = screenOffFinishedAnimatingLocked(reason);
                        }
                        sendNotificationLocked(false, reason);
                        mWaitKeyguardDraw = true;
                    } else {
                        err = 0;
                        mLastTouchDown = 0;
                    }
    ...
    private void updateLightsLocked(int newState, int forceState) {
        ...
        if ((difference & (SCREEN_ON_BIT | SCREEN_BRIGHT_BIT)) != 0) {
            ...
            Slog.d(TAG, "newState=" + newState + ", realDifference=" + realDifference + "SCREEN_ON_BIT=" + SCREEN_ON_BIT);
            if((newState & SCREEN_ON_BIT & realDifference) != 0)
                mDoDelayLightAnimation = true;
            else 
                mDoDelayLightAnimation = false;
            mScreenBrightness.setTargetLocked(brightness, 1, //steps,
    ...
    private void setLightBrightness(int mask, int value) {
        ...
        if ((mask & SCREEN_BRIGHT_BIT) != 0) {
           //if(value == Power.BRIGHTNESS_OFF){
                 if (FeatureOption.MTK_IPO_SUPPORT == true) {
                    if ("1".equals(SystemProperties.get("sys.ipo.shutdown"))) {
                  	    if(mSpew) Slog.d(TAG, "do not turn backlight off/on when/after shutdown with IPO mode!");
                  	    return;
                    }
                 }
            //}
        ...
        if ((mask & BUTTON_BRIGHT_BIT) != 0) {
            if(value != Power.BRIGHTNESS_OFF){
                 if (FeatureOption.MTK_IPO_SUPPORT == true) {
                    if ("1".equals(SystemProperties.get("sys.ipo.shutdown"))) {
                  	    if(mSpew) Slog.d(TAG, "do not turn button on after shutdown with IPO mode!");
                  	    return;
                    }
                 }
            }
        ...
        if ((mask & KEYBOARD_BRIGHT_BIT) != 0) {
            if(value != Power.BRIGHTNESS_OFF){
                 if (FeatureOption.MTK_IPO_SUPPORT == true) {
                    if ("1".equals(SystemProperties.get("sys.ipo.shutdown"))) {
                  	    if(mSpew) Slog.d(TAG, "do not turn keyboard on after shutdown with IPO mode!");
                  	    return;
                    }
                 }
            }
    ...
    class BrightnessState implements Runnable {
        ...
        void setTargetLocked(int target, int stepsToTarget, int initialValue,
            ...
            animating = true;
            if (mPreventScreenOn && mDoDelayLightAnimation)
            {
                // Skip scheduling light ON animation here if preventScreenOn(true) is called
                // by PhoneApp. Instead, delay the light animation when preventScreenOn(false)
                // is called by PhoneApp or timeout (forceReenableScreen)
            }
            else
            {
                if (mSpew) {
                      Slog.i(TAG, "Scheduling light animator!, mDoDelayLightAnimation=" + mDoDelayLightAnimation);
                }
                if(mDoDelayLightAnimation){
                      mScreenOffHandler.removeCallbacks(this);
                      mScreenOffHandler.post(this);
                }else{      
                      mScreenOffHandler.removeCallbacks(mLightAnimator);
                      mScreenOffHandler.post(mLightAnimator);      
                }
            }
        }
        ...
        private void finishAnimationLocked(boolean more, int curIntValue) {
            ...
            if (!more) {
                if (mask == SCREEN_BRIGHT_BIT && curIntValue == Power.BRIGHTNESS_OFF) {
                    clearLightSensorValue();
                    if((!KeyguardViewMediator.isKeyguardEnabled()) || (mScreenOffReason == WindowManagerPolicy.OFF_BECAUSE_OF_PROX_SENSOR))
                    {
                         //turn screen off in default flow, only when keyguard is disabled 
                         //do screen off without keyguard view ready, when backlight is off because of proximity sensor
                         screenOffFinishedAnimatingLocked(mScreenOffReason);
                    }
                    sendNotificationLocked(false, mScreenOffReason);
                    mWaitKeyguardDraw = true;
                }
            }
        }        	   
        ...
        public void run() {	
            ...
            synchronized (mSyncKeyguard) {
                if(!mSyncKeyguard.mKeyguardDoneDraw){
                    if(mSpew) {
                        Slog.d(TAG, "DelayLightAnimator is going to suspend");
                    }
                    try{
                         mSyncKeyguard.wait(1000);                        
                    }catch (InterruptedException e) {}	
                    if(mSyncKeyguard.mKeyguardDoneDraw)	{
                         if(mSpew) Slog.d(TAG, "DelayLightAnimator is resumed, post mLightAnimator");
                    }else{
                         mSyncKeyguard.mKeyguardDoneDraw=true;
                         if(mSpew) Slog.d(TAG, "DelayLightAnimator is timeout, post mLightAnimator");
                    }
                    long now = SystemClock.uptimeMillis();
                    mScreenOffHandler.postAtTime(mLightAnimator, now+500);
                }else{
                    if(mSpew) Slog.d(TAG, "DelayLightAnimator is going to post mLightAnimator without suspend");
                    mScreenOffHandler.post(mLightAnimator);
                }
            }
        }
        ...
        //LightAnimator
        public Runnable mLightAnimator = new Runnable()
        {
            public void run() 
            {
        ...
        try {
            if (mScreenBrightnessOverride >= 0) {
                return mScreenBrightnessOverride;
                    ...
                    && mAutoBrightessEnabled && (!FEATURE_DISABLE_LS_ENABLED || !mIsLowBattery)) {
    ...
    private int applyButtonState(int state) {
        ...
        } else if (mLightSensorButtonBrightness >= 0 && mUseSoftwareAutoBrightness && !FEATURE_ALIGN_BKL_ENABLED) {
    ...
    private int applyKeyboardState(int state) {
        ...
        if (!mKeyboardVisible) {
            ...
            brightness = mButtonBrightnessOverride;
        } else if (mLightSensorKeyboardBrightness >= 0 && mUseSoftwareAutoBrightness && !FEATURE_ALIGN_BKL_ENABLED) {
        ...
        synchronized (mLocks) {
            if (mSpew) {
                Slog.d(TAG, "userActivity mLastEventTime=" + mLastEventTime + " time=" + time
                        ...
                        + " mProximitySensorEnabed=" + mProximitySensorEnabled
                        + " timeoutOverride=" + timeoutOverride
                        + " force=" + force);
            }
            ...
            if (mLastEventTime <= time || force) {
                ...
                if ((mUserActivityAllowed && !mProximitySensorActive) || force) {
                    ...
                    if (eventType == BUTTON_EVENT && (!mUseSoftwareAutoBrightness || FEATURE_ALIGN_BKL_ENABLED)) {
            ...
            if ((mPowerState & BATTERY_LOW_BIT) == 0) {
                ...
                if (mAutoBrightessEnabled && mScreenBrightnessOverride < 0) {
                    if (mAnimateScreenLights) 
                    {
                        ...
                        {
                            if (mDebugLightSensor) Slog.d(TAG, "Skip animation in DIM state");
                        }
                        else
                        {
                            mDoDelayLightAnimation = false;
                            if (mDebugLightSensor) {
                                Slog.d(TAG, "<<<<< FEATURE_ALIGN_BKL >>>>> LCD BKL Changes because of LS");
                            }
                            mScreenBrightness.setTargetLocked(lcdValue, 1,
                                                              INITIAL_SCREEN_BRIGHTNESS, (int)mScreenBrightness.curValue);
                        }
                    }
                }
                if (mButtonBrightnessOverride < 0 && 
                    (!FEATURE_ALIGN_BKL_ENABLED || (mPowerState & BUTTON_BRIGHT_BIT) != 0)) {
                    if (mDebugLightSensor) {
                        Slog.d(TAG, "<<<<< FEATURE_ALIGN_BKL >>>>> BUTTON BKL Changes because of LS");
                    }
                    mButtonLight.setBrightness(buttonValue);
                }
                if ((mButtonBrightnessOverride < 0 || !mKeyboardVisible) && 
                    (!FEATURE_ALIGN_BKL_ENABLED || (mPowerState & KEYBOARD_BRIGHT_BIT) != 0)) {
                    if (mDebugLightSensor) {
                        Slog.d(TAG, "<<<<< FEATURE_ALIGN_BKL >>>>> KEYBOARD BKL Changes because of LS");
                    }
    ...
    {
        boolean isKeyguardNotShowing = KeyguardViewMediator.isKeyguardNotShowing();
        IActivityManager am = ActivityManagerNative.getDefault();
        if((am!=null) && am.testIsSystemReady()&& isKeyguardNotShowing){
            if(mSpew) Slog.w(TAG, "reset mSyncKeyguard in goToSleep()");
            mSyncKeyguard.mKeyguardDoneDraw = false;
        }
    ...
    //workaround for keyguardViewMediator 
    public void enableUserActivityForce(boolean enabled, boolean force) {
         if(mSpew)
             Slog.d(TAG, "enableUserActivityForce enabled=" + enabled + ", force=" + force);
         synchronized (mLocks) {
    ...
    private void setScreenBrightnessMode(int mode) {
        if (mSpew) {
            Slog.d(TAG, "<<<<< setScreenBrightnessMode >>>>> mode=" + mode);
        }
        ...
        if (mUseSoftwareAutoBrightness && mAutoBrightessEnabled != enabled) {
            ...
            //Begin for feature that disable Light Sensor at proper time 
            if (FEATURE_DISABLE_LS_ENABLED && isScreenOn() && !mIsLowBattery && mBootCompleted) { 
                if (mDebugLightSensor) {
                    Slog.d(TAG, "<<<<< FEATURE_DISABLE_LS >>>> LCD Mode Changed: new_mode=" + mAutoBrightessEnabled);
                }
            ...
            //End for feature that disable Light Sensor at proper time
    ...
    void systemReady() {
        ...
        Resources resources = mContext.getResources();
        mUseSoftwareAutoBrightness = mSensorManager != null &&
        ...
        if (mUseSoftwareAutoBrightness) {
            ...
            mIsLowBattery = !mBatteryService.isPowered() && mBatteryService.getBatteryLevel() <= Power.LOW_BATTERY_THRESHOLD;
            if (!FEATURE_DISABLE_LS_ENABLED || (mAutoBrightessEnabled && !mIsLowBattery)) {
    ...
    SensorEventListener mLightListener = new SensorEventListener() {
        public void onSensorChanged(SensorEvent event) {
            synchronized (mLocks) {
                ...
                long milliseconds = SystemClock.elapsedRealtime();
                if (mDebugLightSensor) {
                    Slog.d(TAG, "onSensorChanged: light value: " + value + ", mHighestLightSensorValue:" + mHighestLightSensorValue);
                }
                if(value > mHighestLightSensorValue)
                {
                    int lcdValue = getAutoBrightnessValue(value, mLcdBacklightValues);
                    if(mLightSensorScreenBrightnessWait == lcdValue)
                    {
                       if(mDebugLightSensor)
                           Slog.d(TAG, "lcdValue == mLightSensorScreenBrightnessWait, do nothing");
                       return;
                    }
                    else
                    {
                        if(mDebugLightSensor)
                            Slog.d(TAG, "update mLightSensorScreenBrightnessWait from " + mLightSensorScreenBrightnessWait + ", to current lcdValue:" + lcdValue);
                        mLightSensorScreenBrightnessWait = lcdValue;
                    }
                }else
                {
                    return;
	...
	/* Code added for Java Layer WakeLocks debugging */

    private String mWakeLockBGDbgModeResult;
    private boolean mIsWakeLockBGDbgModeRunning = false;
    private boolean mIsWakeLockSkippedByTag = true;
    private static final String[] WAKELOCK_SKIPPED_TAG = {	
                                    "ActivityManager-Launch", "ActivityManager-Sleep",
                                    "AlarmManager", 
                                    "KeyInputQueue", "KEEP_SCREEN_ON_FLAG", "SCREEN_FROZEN", 
                                    "sleep_broadcast", "Proximity Partial",
                                    "RILJ",
                                    "show keyguard", "keyguard", "keyguardWakeAndHandOff",
                                    "VibratorService",
                                    };

	
    private String getWakeLockFlagLocked(WakeLock wl) {
        String flag = "";
        ...
        if ((wl.flags & PowerManager.PARTIAL_WAKE_LOCK) == PowerManager.PARTIAL_WAKE_LOCK) {
            flag += "PARTIAL_WAKE_LOCK";
            isMultiFlag = true;
        }
        if ((wl.flags & PowerManager.FULL_WAKE_LOCK) == PowerManager.FULL_WAKE_LOCK) {
            flag += isMultiFlag == true ? "|" : "";
            flag += "FULL_WAKE_LOCK";
            isMultiFlag = true;
        } else if ((wl.flags & PowerManager.SCREEN_BRIGHT_WAKE_LOCK) == PowerManager.SCREEN_BRIGHT_WAKE_LOCK) {
            flag += isMultiFlag == true ? "|" : "";
            flag += "SCREEN_BRIGHT_WAKE_LOCK";
            isMultiFlag = true;
        }
        if ((wl.flags & PowerManager.SCREEN_DIM_WAKE_LOCK) == PowerManager.SCREEN_DIM_WAKE_LOCK) {
            flag += isMultiFlag == true ? "|" : "";
            flag += "SCREEN_DIM_WAKE_LOCK";
            isMultiFlag = true;
        }
        if ((wl.flags & PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK) == PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK) {
            flag += isMultiFlag == true ? "|" : "";
            flag += "PROXIMITY_SCREEN_OFF_WAKE_LOCK";
            isMultiFlag = true;
        }
        if ((wl.flags & PowerManager.ACQUIRE_CAUSES_WAKEUP) == PowerManager.ACQUIRE_CAUSES_WAKEUP) {
            flag += isMultiFlag == true ? "|" : "";
            flag += "ACQUIRE_CAUSES_WAKEUP";
            isMultiFlag = true;
		}
        if ((wl.flags & PowerManager.ON_AFTER_RELEASE) == PowerManager.ON_AFTER_RELEASE) {
            flag += isMultiFlag == true ? "|" : "";
            flag += "ON_AFTER_RELEASE";
            isMultiFlag = true;
        }
    ...
    private void getDynamicWakeLockLocked(WakeLock wl, boolean isAcquire) {
        if (!mIsWakeLockBGDbgModeRunning) 
            return ;
		
        if (mIsWakeLockSkippedByTag) {	
            for (int i=0; i < WAKELOCK_SKIPPED_TAG.length; i++) {
                if (wl.tag.equals(WAKELOCK_SKIPPED_TAG[i])) 
                    return ;	
            }
        }
		
        if (isAcquire)	// acquire a WakeLock	
            mWakeLockBGDbgModeResult += "\nAcquired: TAG = " + wl.tag + " , FLAG = " + getWakeLockFlagLocked(wl);
        else	// release a WakeLock 
            mWakeLockBGDbgModeResult += "\nReleased: TAG = " + wl.tag;
    }
    

    public String debugWakeLock() { 
        String result;
        
        synchronized (mLocks) {
            if (mSpew && false) {
                Slog.d(TAG, "debugWakeLock");
            }
            int N = mLocks.size();
            WakeLock wl = null;
            result = "WakeLock List Size = " + N ;
            for (int i=0; i<N; i++) {
                wl = mLocks.get(i);
                if (wl == null) {
                    result +="\nNo." + (i+1) + ": get null pointer";  
                    continue;
                }
                result += "\nNo." + (i+1) + ": TAG = " + wl.tag + " , FLAG = " + getWakeLockFlagLocked(wl);
            }
        }
		
        Slog.d(TAG, "debugWakeLock in ForeGround Mode, result String=" + result);
        return result;
    }

	
    //isStartBgMode: whether start or stop Wake Lock background debug mode
    //isSkipByTag:  whether skip some regular WakeLocks by tag, such as wakelock whose tag is "keyguard"	
    public String debugWakeLockUnderBG(boolean isStartBgMode, boolean isSkipByTag) {
        synchronized (mLocks) {
            if (mSpew && false) {
                Slog.d(TAG, "debugWakeLockUnderBG, isStartBgMode=" + isStartBgMode + " isSkipByTag=" + isSkipByTag);
            }
            //BackGround Mode Start 
            if (isStartBgMode) {
                if (!mIsWakeLockBGDbgModeRunning) {
                    mIsWakeLockBGDbgModeRunning = true;
                    //Whethe skip WakeLock by tag chosed here only !!!
                    mIsWakeLockSkippedByTag = isSkipByTag;
                    mWakeLockBGDbgModeResult = debugWakeLock() + "\n";
                    return "BackGround Mode Starts now.";
                } else {
                    //isSkipByTag take no effect under this scenario
                    return "BackGround Mode Starts already!";
                }	
            //BackGround Mode Stop 
            } else {
                if (mIsWakeLockBGDbgModeRunning) {
                    mIsWakeLockBGDbgModeRunning = false;
                    mWakeLockBGDbgModeResult += "\n\n" + debugWakeLock();
                    return mWakeLockBGDbgModeResult;
                } else {
                    return "BackGround Mode Stops already.";
                }
            }
        }
    }	
