...
class PowerManagerService extends IPowerManager.Stub
    ...
    private int mLightSensorPendingLevel = -1;
    private int mLightSensorLastLevel = -1;
    ...
    */
	...
	private int getLuxLevel(int sensorValue)
	{
            int i;
	...
	private int Level2Brightness(int level,int[] values) {
		try {
			return values[level];
        } catch (Exception e) {
            // guard against null pointer or index out of bounds errors
            Slog.e(TAG, "Level2Brightness", e);
            return 255;
        }
	}
    ...
    private Runnable mAutoBrightnessTask = new Runnable() {
        public void run() {
            synchronized (mLocks) {
                int value = mLightSensorPendingLevel;
                if (value >= 0) {
                    mLightSensorPendingLevel = -1;
    ...
    private void dockStateChanged(int state) {
        synchronized (mLocks) {
            ...
            if ((mPowerState & SCREEN_ON_BIT) != 0) {
                ...
                int value = (int)mLightSensorLastLevel;
                mLightSensorLastLevel = -1;
                lightSensorChangedLocked(value);
            }
        }
    }
    ...
    private void lightSensorChangedLocked(int value) {
        ...
        if (mLightSensorLastLevel != value) {
            mLightSensorLastLevel = value;
            if ((mPowerState & BATTERY_LOW_BIT) == 0) {
                ...
                int lcdValue = Level2Brightness(value, mLcdBacklightValues);
                int buttonValue = Level2Brightness(value, mButtonBacklightValues);
                int keyboardValue;
                if (mKeyboardVisible) {
                    keyboardValue = Level2Brightness(value, mKeyboardBacklightValues);
                } else {
                    keyboardValue = 0;
                }
    ...
    SensorEventListener mLightListener = new SensorEventListener() {
        public void onSensorChanged(SensorEvent event) {
            synchronized (mLocks) {
                ...
                int level = getLuxLevel(value);
                
                long milliseconds = SystemClock.elapsedRealtime();
                if (mDebugLightSensor) {
                    Slog.d(TAG, "onSensorChanged: light value: " + value);
                }
                if (level!=mLightSensorLastLevel) {
					mHandler.removeCallbacks(mAutoBrightnessTask);
					if (mLightSensorPendingLevel != level) {
						if (mLightSensorPendingLevel == -1 ||
								milliseconds < mLastScreenOnTime + mLightSensorWarmupTime) {
							// process the value immediately if screen has just turned on
							lightSensorChangedLocked(level);
						} else {
							// delay processing to debounce the sensor
							mLightSensorPendingLevel = level;
